		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				$(".scroll").click(function(event){		
					...
					<span class="math-inline">\('html,body'\)\.animate\(\{scrollTop\:</span>(this.hash).offset().top},900);
				});
			});
</script>
		<script>
...
  };
		...
		<div class="section main main-raised">
			<div class="container">
				<?php
					$con = mysqli_connect("localhost", "pma", "pma", "onlineshop");  // Replace with your database connection details

					if (!$con) {
						die("Connection failed: " . mysqli_connect_error());
					}

					$product_id = filter_var($_GET['p'], FILTER_SANITIZE_NUMBER_INT);  // Validate and sanitize user input
					...
					$sql = "SELECT * FROM products AS P, categories AS C WHERE P.product_cat = C.cat_id AND P.product_id = ?";

					$stmt = mysqli_prepare($con, $sql);  // Prepare the statement
					mysqli_stmt_bind_param($stmt, "i", $product_id);  // Bind the parameter
